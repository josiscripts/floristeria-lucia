...
try {
  ...
  (data || []).forEach((opt) => {
    ...
    console.log(
      `  - ${opt.name}: €${opt.price_amount} [SKU: ${opt.sku}] ghl_price_id=${ghlStatus}`,
    );
  });
} catch (error) {
  console.log(`Error: ${error.message}`);
...
try {
  const { data, count, error } = await supabase.from("product_images").select("*").limit(1);
  ...
  if (error) {
    ...
    const response = await supabase.from("product_images").select("*").limit(1);
...
try {
  const { data, count } = await supabase.from("color_variants").select("id, product_id, name");
