/**
 * TEST ENDPOINT ONLY - For BLOQUE 4 execution without auth
 * POST /api/sync-catalog-test
 *
 * ⚠️  This endpoint bypasses authentication for development/testing.
...
const POST = async (request: any) => {
  try {
    ...
    console.error("[API] /api/sync-catalog-test error:", message);
    return json(
...
export const Route = createFileRoute("/api/sync-catalog-test")({
  server: {
    handlers: {
