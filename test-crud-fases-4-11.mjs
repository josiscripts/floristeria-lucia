...
async function getOrCreateTestProduct() {
  try {
    ...
    const createResponse = await makeAPICall("POST", "/api/products", {
...
async function ejecutarFase4(testProduct) {
  ...
  try {
    ...
    // EDITAR
    console.log("\nEditando producto...");
    const editResponse = await makeAPICall("PUT", `/api/products/${ghlProductId}`, {
    ...
    const success =
      productAfter.name === "TEST BLOQUE 4 — CRUD REAL EDITADO" && duplicateCount === 1;
...
async function ejecutarFase5(testProduct) {
  ...
  try {
    ...
    // Eliminar
    console.log("\nEliminando producto...");
    const deleteResponse = await makeAPICall("DELETE", `/api/products/${ghlProductId}`);
...
async function ejecutarFase9() {
  ...
  try {
    ...
    for (const product of testProducts || []) {
      ...
      const response = await makeAPICall("DELETE", `/api/products/${product.ghl_product_id}`);
