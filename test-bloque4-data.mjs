...
const GHL_TOKEN =
  process.env.GHL_PRIVATE_INTEGRATION_TOKEN || "pit-0cf65f40-51a4-4e28-9793-9eb8421e2291";
...
async function verifySupabase() {
  ...
  try {
    ...
    const { data: orphanOptions, error: orphanOptError } = await supabase.rpc(
      "check_orphan_product_options",
    );
    ...
    if (orphanOptError && orphanOptError.code !== "PGRST201") {
      ...
      console.log(
        `✓ Orphan product_options: ${orphanCount} [${orphanCount === 0 ? "DEMOSTRADO" : "FAILED"}]`,
      );
    }
    ...
    if (listError) {
      ...
      firstProducts.forEach((p, i) => {
        console.log(
          `${i + 1}. ${p.name} (id: ${p.id.substring(0, 8)}..., ghl: ${p.ghl_product_id})`,
        );
      });
