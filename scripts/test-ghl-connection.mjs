...
async function testGHLConnection() {
  ...
  try {
    ...
    console.log(`   Token: ${token.substring(0, 10)}...${token.substring(token.length - 10)} (${token.length} chars)`);
    console.log(`   Location ID: ${locationId.substring(0, 8)}...${locationId.substring(locationId.length - 4)} (${locationId.length} chars)\n`);
    ...
    if (data.products && data.products.length > 0) {
      ...
      data.products.slice(0, 3).forEach((product, idx) => {
        ...
        if (product.description) console.log(`      Description: ${product.description.substring(0, 60)}...`);
