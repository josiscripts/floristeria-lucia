#!/usr/bin/env node
/**
 * FASE 3A: Clean all metadata and re-populate correctly
 */
...
const catalogProducts = [
  ...
  { id: "centro-orquideas-variadas", name: "Centro de Orquídeas Variadas", category: "plantas", priceMin: 80 },
  { id: "centro-orquidea-blanca", name: "Centro Orquídea Blanca", category: "plantas", priceMin: 80 },
  ...
  { id: "caja-rosas-eternas", name: "Caja de Rosas Eternas", category: "rosas-eternas", priceMin: 40 },
  ...
  { id: "jarron-cristal-1", name: "Jarrón de Cristal Nº 1", category: "complementos", priceMin: 1.5 },
  { id: "jarron-cristal-2", name: "Jarrón de Cristal Nº 2", category: "complementos", priceMin: 5 },
  { id: "chocolate-belga-pequena", name: "Chocolate Belga Pequeña", category: "complementos", priceMin: 12.5 },
  { id: "chocolate-belga-grande", name: "Chocolate Belga Grande", category: "complementos", priceMin: 15 },
  { id: "oso-peluche", name: "Oso de Peluche", category: "complementos", priceMin: 12.5 },
  { id: "oso-peluche-corazon", name: "Oso de Peluche Corazón", category: "complementos", priceMin: 12 },
  { id: "macetero-violeta-orquidea", name: "Macetero Violeta Orquídea", category: "complementos", priceMin: 4.5 },
  { id: "macetero-blanco-orquidea", name: "Macetero Blanco Orquídea", category: "complementos", priceMin: 4.5 },
  ...
  { id: "centro-almohadon-f22", name: "Centro almohadón F22", category: "condolencias", priceMin: 75 },
  ...
  { id: "centro-almohadon-xxl", name: "Centro almohadón XXL", category: "condolencias", priceMin: 230 },
...
function matchWithCatalog(ghlProduct) {
  ...
  const exact = catalogProducts.find(p =>
    normalizeString(p.name) === normalizeString(ghlName)
  );
...
async function main() {
  ...
  console.log('\n═══════════════════════════════════════════════════════');
  console.log('STEP 1: LIMPIAR TABLA');
  console.log('═══════════════════════════════════════════════════════\n');
  ...
  const { error: deleteError } = await supabase
    .from('product_metadata')
    .delete()
    .neq('ghl_product_id', null);

  if (deleteError) {
    console.log(`  ❌ Error al limpiar: ${deleteError.message}`);
    process.exit(1);
  }
  ...
  console.log('═══════════════════════════════════════════════════════');
  console.log('STEP 2: FETCH GHL PRODUCTS');
  console.log('═══════════════════════════════════════════════════════\n');
  ...
  console.log(`  Normalized: ${normalizedProducts.length}\n`);

  console.log('═══════════════════════════════════════════════════════');
  console.log('STEP 3: MATCH Y GENERAR RECORDS');
  console.log('═══════════════════════════════════════════════════════\n');
  ...
  for (const ghl of normalizedProducts) {
    if (!ghl.id || ghl.id === 'undefined') continue;
  ...
  console.log('═══════════════════════════════════════════════════════');
  console.log('STEP 4: INSERTAR EN SUPABASE');
  console.log('═══════════════════════════════════════════════════════\n');

  let successCount = 0;
  let errorCount = 0;

  // Insert matched with better error handling
  for (const m of matched) {
    ...
    if (error) {
      console.log(`  ❌ ${m.ghlName}: ${error.message}`);
      errorCount++;
    } else {
      console.log(`  ✓ ${m.ghlName}`);
      successCount++;
    }
  }
  ...
  // Insert orphans (with NULL status instead of needs_review, which violates constraint)
  for (const o of orphans) {
    ...
    if (error) {
      console.log(`  ⚠ ${o.ghlName} (orphan): ${error.message}`);
      errorCount++;
    } else {
      console.log(`  ⚠ ${o.ghlName} (orphan)`);
      successCount++;
    }
  }
  ...
  console.log(`  Total insertados: ${successCount}`);
  ...
  console.log(`  Registros finales en Supabase: ${finalCount}\n`);

  if (finalCount === 68) {
    console.log('✅ ÉXITO: 68 registros en Supabase');
  } else {
    console.log(`❌ ADVERTENCIA: Esperado 68, obtuve ${finalCount}`);
  }
}
