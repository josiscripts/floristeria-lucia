...
const catalogModule = await import("../src/data/catalog.ts", { assert: { type: "module" } }).catch(
  () => null,
);
...
for (const catalogProduct of catalogProducts) {
  ...
  if (ghlIds.has(catalogProduct.id)) {
    ...
    // Try name match
    const nameMatch = ghlProducts.find(
      (p) => p.name && p.name.toLowerCase() === catalogProduct.name.toLowerCase(),
    );
...
for (const ghlProduct of ghlProducts) {
  if (!catalogIds.has(ghlProduct.id) && !possibleMatches.some((m) => m.ghl.id === ghlProduct.id)) {
...
if (matches.length > 0) {
  ...
  console.log(
    `  catalog.ts: ${sample.catalog.id} - ${sample.catalog.name} ($${sample.catalog.priceMin})`,
  );
...
console.log("");
console.log(
  `Coverage: ${Math.round(((matches.length + possibleMatches.length) / catalogProducts.length) * 100)}%`,
);
console.log("");
