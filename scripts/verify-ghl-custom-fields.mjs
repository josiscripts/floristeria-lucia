...
async function explore() {
  ...
  if (actualRes.ok && actualRes.data?.products?.length > 0) {
    ...
    const nativeFields = [
      '_id', 'locationId', 'name', 'description', 'image',
      'category', 'status', 'price', 'cost', 'sku',
      'productType', 'availableInStore', 'variants',
      'isTaxesEnabled', 'taxes', 'trackProductInventory'
    ];
    ...
    unknownFields.forEach(f => {
      console.log(`  ? ${f}: ${typeof product[f]} = ${JSON.stringify(product[f]).substring(0, 50)}`);
    });
  }
