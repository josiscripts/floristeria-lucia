...
async function main() {
  ...
  while (true) {
    const { data } = await request('GET', `/products/?locationId=${GHL_LOCATION_ID}&limit=${limit}&skip=${skip}`);
  ...
  for (let i = 0; i < allProducts.length; i++) {
    ...
    if (status === 200 || status === 204) {
      ...
      failed++;
      console.log(`  [${i + 1}/${allProducts.length}] ❌ Failed to delete ${id.substring(0, 12)} (${status})`);
    }
  }
